...
int main(){
    ...
    vector<HeuristicsConfig> random_opponents_arr(200);
    for(int i=0; i<100; i++){
    ...
    for(int i=0; i<N1; i++){
        for(int j=0; j<N2; j++){
            ...
            current_config.def_pieces_weight = -0.5 + i*0.1;
            current_config.king_free_moves_weight = -0.5 + j*0.1;
            config_arr[idx] = current_config;
        }
    }

    grid_search(config_arr, random_opponents_arr, 3, "data_new/grid_21x21_defpieces_vs_kingfreedom.txt");
    // HeuristicsConfig config;
