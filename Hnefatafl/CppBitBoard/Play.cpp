...
int main(){
    ...
    for(int i=0; i<N1; i++){
        for(int j=0; j<N2; j++){
            ...
            current_config.king_neighboring_enemies_weight = -0.5 + i*0.1;
            current_config.king_neighboring_allies_weight = -0.5 + j*0.1;
            config_arr[idx] = current_config;
        }
    }

    grid_search(config_arr, random_opponents_arr, 4, "data_new/grid_31x41_def_enemies_allies.txt");
