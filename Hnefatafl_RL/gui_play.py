...
Displays:
- Board state
- Raw network policy probabilities (piece selection and move destinations)
- Value evaluation from current player's perspective

Usage:
    python gui_play.py <checkpoint_path>
            ...
            for row in range(BOARD_SIZE):
                for col in range(BOARD_SIZE):
                    ...
                    if prob > 0.001:
                        ...
                        prob_text = self.font_small.render(f"{prob*100:.1f}%", True, BLACK)
                        text_rect = prob_text.get_rect(center=(x + SQUARE_SIZE//2, y + SQUARE_SIZE - 15))
...
def main():
    ...
    gui = BrandubhGUI(args.checkpoint)
    gui.run()
