    ...
    def _check_capture_in_direction(self, r: int, c: int, dr: int, dc: int, piece: int):
        ...
        if target_is_king:
            ...
            if self.king_capture_pieces == 2:
                nr2, nc2 = nr + dr, nc + dc
                if 0 <= nr2 < 11 and 0 <= nc2 < 11 and (self.board[nr2, nc2] == ATTACKER or self._is_hostile_square(nr2, nc2)):
    ...
    def _is_king_captured(self, king_r: int, king_c: int) -> bool:
        ...
        if self.king_capture_pieces == 2:
            ...
            if (0 <= king_c - 1 and self.board[king_r, king_c - 1] == ATTACKER and
                king_c + 1 < 11 and self.board[king_r, king_c + 1] == ATTACKER):
                return True
            # Check vertical
            if (0 <= king_r - 1 and self.board[king_r - 1, king_c] == ATTACKER and
                king_r + 1 < 11 and self.board[king_r + 1, king_c] == ATTACKER):
                return True
            return False
            ...
            for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if 0 <= nr < 11 and 0 <= nc < 11 and self.board[nr, nc] == ATTACKER:
                    attacker_count += 1
            ...
            for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                nr, nc = king_r + dr, king_c + dc
                if not (0 <= nr < 11 and 0 <= nc < 11) or self.board[nr, nc] != ATTACKER:
                    return False
            return True
