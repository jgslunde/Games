"""
Tablut (9x9 Tafl) game implementation.
...
class Tablut:
    def __init__(self, 
        ...
        """
        Initialize Tablut game with tunable rules.
    ...
    def _setup_board(self):
        """Initialize the starting position for Tablut."""
        # King at center
        self.board[4, 4] = KING
        
        # Defenders around king (cross pattern)
        defenders = [
            (3, 4), (4, 3), (4, 5), (5, 4),  # Adjacent to king
            (2, 4), (4, 2), (4, 6), (6, 4)   # One square away
        ]
        for r, c in defenders:
            self.board[r, c] = DEFENDER
        
        # Attackers on edges (standard Tablut layout)
        attackers = [
            # Top edge
            (0, 3), (0, 4), (0, 5),
            (1, 4),
    ...
    def get_state(self) -> np.ndarray:
        ...
        state[3] = np.full((9, 9), self.current_player, dtype=np.float32)
        return state
    ...
    def _check_capture_in_direction(self, r: int, c: int, dr: int, dc: int, piece: int):
        ...
        if target_is_king:
            ...
            if self.king_capture_pieces == 2:
                nr2, nc2 = nr + dr, nc + dc
                if 0 <= nr2 < 9 and 0 <= nc2 < 9 and (self.board[nr2, nc2] == ATTACKER or self._is_hostile_square(nr2, nc2, KING)):
    ...
    def clone(self):
        ...
        new_game = Tablut.__new__(Tablut)
